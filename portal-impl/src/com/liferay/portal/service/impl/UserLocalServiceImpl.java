...
public class UserLocalServiceImpl extends UserLocalServiceBaseImpl {
	...
	public void test2(long userGroupId, long[] userIds) throws PortalException {
		...
		for (long userId : userIds) {
			...
			for (long s : sites) {
				...
				long[] gorupIduserIds = getGroupUserIds(s);

				if (!contains &&
					!LongStream.of(gorupIduserIds).anyMatch(x -> x == userId)) {
